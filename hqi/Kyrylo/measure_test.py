...
def main():
    ...
    for i in range(len(list_position)):
        job_obs=circ.to_job("OBS",observable=Observable.z(list_position[i],nbqbits))
    ...
    val_test=np.zeros(len(list_position))
    for i in range(len(list_position)):
        job_obs=circ.to_job("OBS",observable=Observable.y(list_position[i][0],nbqbits)*Observable.y(list_position[i][1],nbqbits))
        result=qpu.submit(job_obs)
        val_test[i]=result.value
