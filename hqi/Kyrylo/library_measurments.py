...
def one_point_corr(circ,direction,list_position,qpu):
    """Computes the one-point correlation function <sigma_i> for each position in list_position, in the specified measurement basis. It is done by sampling the final state and accumulating probabilities weighted by the sign of the measure spin values.

    <sigma_i> = sum over samples of [ sign(bit_i) * probability ]

    where sign is -1 if the bit is 1 (spin down) and +1 if the bit is 0 (spin up).

    Parameters:
      circ       Circuit    Input quantum circuit.
      direction      str        Measurement basis: 'X', 'Y', or 'Z'
                                  'X' --> applies RY(-pi/2) to each qubit in qbits
                                  'Y' --> applies RX(+pi/2) to each qubit in qbits
                                  'Z' --> no gate applied (standard Z measurement)
      list_position  list       List of qubit indices for which to compute <sigma_i>
                                Each element is a single integer index.
      qpu            QPU        Quantum processing unit used to run the circuit.
...
def two_point_corr(circ,direction,list_position,qpu):
    """Computes the two-point correlation function <sigma_i * sigma_j> for each pair of positions in list_position, in the specified measurement basis.

    <sigma_i * sigma_j> = sum over samples of [ sign(bit_i * bit_j) * probability ]

    The sign of the product is determined by np.prod() on the selected bits
    (already mapped to +1/-1). If the product is -1, the probability is subtracted;
    if +1, it is added.
